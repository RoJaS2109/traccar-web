...
public class TaipProtocolDecoder extends BaseProtocolDecoder {
    ...
    // Pattern for Rinho RCR messages: extract DEG_DEG coordinates from the body
    private static final Pattern RCR_COORD_PATTERN = new PatternBuilder()
            .number("([-+]dd)(d{5})")            // latitude:  sign + 2-degree + 5-minutes
            .number("([-+]ddd)(d{5})")           // longitude: sign + 3-degree + 5-minutes
            .compile();

    private Position decodeRcr(
            Channel channel, SocketAddress remoteAddress, String sentence) throws Exception {

        // Extract body between "RCR" and the first ';' (attribute section)
        ...
        // Find coordinates within the body
        Parser parser = new Parser(RCR_COORD_PATTERN, body);
        if (!parser.find()) {
            return null;
        }
    ...
    protected Object decode(
        ...
        // Handle Rinho RCR messages (proprietary format based on TAIP)
        if (sentence.startsWith("RCR")) {
            return decodeRcr(channel, remoteAddress, sentence);
        }
